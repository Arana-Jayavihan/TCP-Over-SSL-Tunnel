...
#     try:
#         tune_socket(client)  # only tune real socket
#         data = client.recv(2)
#         if len(data) < 2:
#             client.close()
#             return
...
#             Thread(target=handle_socks, args=(client, transport, stop_event, bufsize), daemon=True).start()
...
#     if transport is not None or transport.is_active():
#         Thread(target=start_socks_server, args=(config['settings']['local_ip'], config['settings']['socks_port'], transport, stop_event), daemon=True).start()
...
# def keep_ssh_alive(config, stop_event, restart_event):
#     ssh_client = None
#     while not stop_event.is_set():
#         try:
#             if ssh_client is None:
#                 ssh_client = initSSH(config, stop_event)
            
#             transport = ssh_client.get_transport()
#             if transport is None or not transport.is_active():
#                 print("[!] SSH connection dropped, reconnecting...")
#                 ssh_client.close()
#                 ssh_client = None
#             sleep(5)
#         except Exception as e:
