...
def initSSH(config):
    ...
    connect_kwargs = {
        ...
        'port': int(config['ssh']['stun_port']),
        'username': config['account']['username'],
        'sock': proxy_sock,
        'compress': True,
    }

    # SSH key authentication (priority: env var > config file)
    key_file = os.environ.get('TCP_TUNNEL_SSH_KEY') or config['ssh'].get('key_file', '').strip()
    if key_file and os.path.exists(key_file):
        connect_kwargs['key_filename'] = key_file
        print(f"[+] Using SSH key: {key_file}")
    else:
        # Password authentication (priority: env var > config file)
        password = os.environ.get('TCP_TUNNEL_PASSWORD') or config['account'].get('password', '').strip()
        if password:
            connect_kwargs['password'] = password
        else:
            raise Exception("No SSH authentication method configured (key_file or password required)")
